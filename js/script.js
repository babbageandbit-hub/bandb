...
(function ($) {
	...
	function headerStyle() {
		if ($('.main-header').length) {
			...
			const siteHeader = $('.header-style-one');
	...
	// Header Search - Use event delegation to avoid double-binding
	if ($(".search-btn").length) {
		$(document).off("click", ".search-btn").on("click", ".search-btn", function () {
			$(".main-header").addClass("moblie-search-active");
		});
		$(document).off("click", ".close-search, .search-back-drop").on("click", ".close-search, .search-back-drop", function () {
  ...
  if ($('.project-image-slider').length) {
   ...
   if ($('.accordion-box').length) {
        $(".accordion-box").on('click', '.acc-btn', function () {
            ...
            const accBtn = $(this);
            const accContent = accBtn.next('.acc-content');
   ...
   if ($('.testimonial-slider').length > 0) {
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		const serviceBlocks = document.querySelectorAll(".service-block-four .inner-box");
		...
		serviceBlocks.forEach(block => {
			...
			serviceBlocks.forEach(item => item.classList.remove("active"));
    ...
    if ($(".mouseCursor").length > 0) {
        function itCursor() {
            const myCursor = jQuery(".mouseCursor");
            if (myCursor.length) {
                if ($("body")) {
                    const e = document.querySelector(".cursor-inner"),
                        t = document.querySelector(".cursor-outer");
                    let n, i = 0, o = !1;
                    // Use addEventListener instead of direct window.onmousemove assignment to avoid breaking other listeners
  ...
  block.addEventListener(
	...
	if ($(".dial").length) {
		$(".dial").appear(
			function () {
				const elm = $(this);
				const color = elm.attr("data-fgColor");
